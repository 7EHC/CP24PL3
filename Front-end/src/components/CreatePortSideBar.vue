...
import { DotsVerticalIcon, PencilAltIcon, PencilIcon, TrashIcon } from '@heroicons/vue/outline';
...
<template>
  <div class="flex overflow-y-auto">
    ...
    <div class="w-60 h-screen bg-zinc-800 p-4 text-white sticky top-0">
      ...
      <button @click="actionToggle('create')" class="create bg-yellow-400 text-zinc-900 font-bold w-full px-4 py-2 rounded-2xl mb-4 hover:bg-yellow-300 duration-300">
        + CREATE PORTFOLIO
      </button>

      <div @click="activeEditDelete" class=" px-2 py-1 float-right flex items-center space-x-1  cursor-pointer transition hover:bg-zinc-600 duration-300 rounded-md"
      ...
      <ul class="space-y-2 mt-12  overflow-y-auto">
        ...
        <div class="flex justify-between items-center">
          ...
          <div v-if="editDelete === true" class="flex space-x-2">
            <PencilAltIcon @click="actionToggle('update',portfolio._id,portfolio.portfolio_name)" class="w-6 h-6 text-blue-500 hover:text-blue-300 duration-200" />
            <TrashIcon @click="actionToggle('delete',portfolio._id)" class="w-6 h-6 text-red-500 hover:text-red-300 duration-200" />
          </div>
        </div>
        </li>
      </ul>
...
<!-- Modal using Teleport -->
    ...
    <div v-if="showModal === true" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div v-if="createDeleteVaue === 'create'" class="bg-white p-6 rounded-lg w-80">
      ...
      <div v-if="createDeleteVaue === 'delete'" class="bg-white p-6 rounded-lg w-960">
        <h3 class="text-xl font-bold mb-4 text-red-600">Delete Portfolio</h3>
          <p class="text-gray-700">Are you sure you want to delete this portfolio? </p>

          <div class="flex justify-end space-x-3 mt-6">
            <button @click="closeModal" class="px-4 py-2 rounded-lg text-gray-500 hover:text-gray-700">
              Cancel
            </button>
            <button @click="confirmDelete" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 font-bold duration-200">
              Delete
            </button>
          </div>
      </div>

      <div v-if="createDeleteVaue === 'update'" class="bg-white p-6 rounded-lg w-960">
        <h3 class="text-xl font-bold mb-4 text-blue-600">Rename Portfolio</h3>
  
        <form @submit.prevent="confirmRename">
          <div class="mb-4">
            <label for="portfolioName" class="block text-sm font-medium text-gray-700">New Portfolio Name</label>
            <input 
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button @click="closeModal" type="button" class="px-4 py-2 rounded-lg text-gray-500 hover:text-gray-700">
              Cancel
            </button>
            <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-bold duration-200">
              Rename
            </button>
          </div>
        </form>
